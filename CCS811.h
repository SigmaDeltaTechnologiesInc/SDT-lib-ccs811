...
#define CCS811_I2C_ADDR                 0xB4    //This is algety left shifted 0x5B, if using other address, should be 0xB4 which is 0x5A left shifted.
...
#define CCS811_REG_BASELINE             0x11    //The encoded current baseline value can be get.A previously saved encoded baseline can be written.
...
class CCS811 {
    public:
        ...
        int getData(uint16_t *ECO2, uint16_t *TVOC);
        bool checkHW();
        bool softRest();
        char getStatus(bool flag);
        bool getMeas();
        bool getError();
    protected:
        I2C _i2c;
